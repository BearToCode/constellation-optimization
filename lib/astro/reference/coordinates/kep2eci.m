function car = kep2eci(kep, mu)
    % kep2eci: converts Keplerian orbital elements to ECI state vectors.
    %
    %  Inputs:
    ...
    if size(kep, 2) > 1
        car = arrayfun(@(i) kep2eci(kep(:, i), mu), 1:size(kep, 2), 'UniformOutput', false);
        car = cell2mat(car);
        return;
    end
