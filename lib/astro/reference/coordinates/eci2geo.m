function cor = eci2geo(car, unix)
    ...
    %
    %  Outputs:
    %   cor: a 3x1 vector containing the geographic coordinates
    %       [latitude, longitude, radius]'

    % If using matrices, use arrayfun to apply the conversion to each column of car and unix.
    if size(car, 2) > 1
        cor = arrayfun(@(i) eci2geo(car(:, i), unix(i)), 1:size(car, 2), UniformOutput = false);
        cor = cell2mat(cor);
        return
    end
